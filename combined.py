...
def entropy(rows):
    ...
    for key, value in counts.items():
        #print(key,",",value)
        child_entropy += ((value/len(rows))*((math.log(value/len(rows)))/math.log(2)))*(-1)
...
def info_gain(left, right, current_uncertainty):
    ...
    return current_uncertainty - prob * entropy(left) - (1 - prob) * entropy(right)
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        for i in rows:
            default[i[-1]] += 1
        self.majority_class = (max(default.items(), key=lambda x: x[1])[0])                     
        self.id = id                                        
        self.depth = depth                                  
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        print (spacing + 'Node Class:' , node.majority_class)    
...
def computeAccuracy(rows, node):
    ...
    for row in rows:
        trueLabel = row[-1]
        predicted = classify(row, node)
#        print("Test: " + predicted + ", Label: " + trueLabel)
        if predicted == row[-1]:
            numAccurate += 1
...
trainDF, testDF = model_selection.train_test_split(df, test_size=0.4)
