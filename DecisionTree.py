...
def info_gain(left, right, current_uncertainty):
    ...
    return current_uncertainty - p2 / p0 * entropy (left) - p1 / p0 * entropy(right)
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        #print (spacing + "Predict", max(node.predictions.items(), key=operator.itemgetter(1))[0])
        print(node.label)
        return
